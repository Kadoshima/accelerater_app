...
import 'dart:collection'; // Queue用に追加
...
class _BLEHomePageState extends State<BLEHomePage> {
  ...
  // Azure Blob Storageコンテナの存在確認と作成
  Future<void> _ensureContainerExists() async {
    try {
      // Azure Storageクライアントを初期化
      final storage = azblob.AzureStorage.parse(azureConnectionString);

      print('Azure Blob Storageコンテナを確認/作成: $containerName');

      // 単純にコンテナ内のblobを一覧取得することでコンテナの存在確認
      try {
        // コンテナ内のファイルを取得しようとする（コンテナが存在するか確認）
        await storage.getBlob('$containerName/test.txt');
        print('コンテナ確認済み: $containerName');
      } catch (e) {
        print('コンテナアクセスエラー詳細: ${e.toString()}');

        // エラーメッセージに基づいて処理
        if (e.toString().contains('BlobNotFound') ||
            e.toString().contains('ContainerNotFound') ||
            e.toString().contains('ResourceNotFound')) {
          // ファイルが見つからないのは正常（コンテナは存在する可能性がある）
          print('コンテナは存在するがファイルが見つかりません。このエラーは無視します。');
        } else if (e.toString().contains('ContainerNotFound') ||
            e.toString().contains('The specified container does not exist')) {
          // コンテナ自体が存在しない場合は空ファイルをアップロードしてコンテナを作成
          print('コンテナが存在しないため作成を試みます...');

          try {
            // 空のファイルをアップロードしてコンテナを暗黙的に作成
            await storage.putBlob(
              '$containerName/init.txt',
              bodyBytes:
                  Uint8List.fromList(utf8.encode('Container initialization')),
              contentType: 'text/plain',
            );
            print('コンテナ作成完了: $containerName');
          } catch (createError) {
            print('コンテナ作成エラー詳細: ${createError.toString()}');
            throw createError; // 作成失敗時は例外を再スロー
          }
        } else {
          // その他のエラー（権限問題など）
          throw e;
      ...
      print('Azure Blob Storageコンテナ確認/作成エラー詳細: ${e.toString()}');
      throw e; // エラーを上に伝播
    }
  }
...
class StepDetector {
  ...
  // 自己相関計算用 (BPM計算方式変更により不要になる可能性)
  // final List<double> accelerationHistory = [];
  // final int _historySize = 300; // 5秒分（60Hz想定）
  // final int _minLag = 30; // 0.5秒（30samples @ 60Hz）
  // final int _maxLag = 180; // 3秒（180samples @ 60Hz）

  // BPM計算結果
  double? _lastCalculatedBpm;
  double _currentReliabilityScore = 0.0;
  // int _consecutiveValidCalculations = 0; // 不要になる
  ...
  void addAccelerationData(double value, int timestamp) {
    ...
    // 履歴データにも追加 (自己相関を使わないなら不要)
    // accelerationHistory.add(value);
    // if (accelerationHistory.length > _historySize) {
  ...
  bool detectStep(double value, int timestamp) {
    ...
    // print(
    // '📊 閾値判定: 値=${value.toStringAsFixed(3)}, 平均=${avg.toStringAsFixed(3)}, 閾値=${_adaptiveThreshold.toStringAsFixed(3)}');

    // ピーク検出ロジック
    if (!_isPotentialPeak && value > avg + _adaptiveThreshold) {
      // 潜在的なピーク開始
      _isPotentialPeak = true;
      // print('📈 潜在的なピーク検出開始'); // デバッグ出力削減
      return false;
      ...
      if (_lastPeakTimestamp > 0 &&
        ...
        // print(
        //     '⏱️ ピーク間隔が短すぎます: ${timestamp - _lastPeakTimestamp}ms < ${_minTimeBetweenSteps}ms'); // デバッグ出力削減
        return false;
      }
      ...
      // print(
      //     '👣 ステップ検出! タイムスタンプ: $timestamp, 合計ステップ: ${stepTimestamps.length}, 信頼度: ${(confidence * 100).toStringAsFixed(1)}%'); // デバッグ出力削減
  ...
  double? calculateBpm() {
    if (stepTimestamps.length < 2) {
      // print('📊 BPM計算: ステップ数不足 (${stepTimestamps.length}/2)');
      return null;
    }
    ...
    for (int i = 1; i < stepTimestamps.length; i++) {
      ...
      // 極端な間隔は除外する（例: 200ms未満 or 2000ms超過 -> 150BPM上限に対応する400ms未満、30BPM下限に対応する2000ms超過は除外）
    ...
    if (intervals.isEmpty) {
      // print('📊 BPM計算: 有効な間隔データがありません (${stepTimestamps.length}ステップ)');
      return null; // 有効な間隔がない場合はnullを返す
    }
    ...
    if (averageInterval <= 0) {
      // print('📊 BPM計算: 平均間隔が0以下です');
      return null;
    }

    // ミリ秒から分への変換：60000ms = 1分
    final bpm = 60000 / averageInterval;

    // print(
    //     '📊 BPM計算 (平均間隔): 有効間隔数=${intervals.length}, 平均=${averageInterval.toStringAsFixed(0)}ms, BPM=${bpm.toStringAsFixed(1)}');
  ...
  // 自己相関を使ったBPM計算は削除
  /*
  double? _calculateBpmFromAutocorrelation() {
    // ... (略) ...
  }
  */
  ...
  void reset() {
    ...
    _currentReliabilityScore = 0.0;
    // _consecutiveValidCalculations = 0;
    print("StepDetector reset.");
  }

  // 非同期で歩行検出を行うメソッド
  // 引数を magnitude から accelerationValue に変更
